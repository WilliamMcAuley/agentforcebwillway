...
# Note: The test below relys upon static assets (for the rendered templates), so require that either:
...
class ExampleTest(TestCase):
    ...
    def test_db_page(self):
        ...
        self.assertEqual(len(first_response.context["accounts"]), 1)

        second_response = self.client.get("/db/")
        self.assertEqual(second_response.status_code, 200)
        self.assertEqual(len(second_response.context["accounts"]), 2)
